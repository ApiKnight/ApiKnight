...
const Mock: React.FunctionComponent<IMockProps> = (props) => {
  ...
  const openModal = () =>{
    getMockList(project_id, api_id).then(res=>{
      res.data.code === 200
      ?
      setMockList(res.data.data.list)
      :
      ''
    })
    setIsModalOpen(true)
  ...
  return (
    ...
    <List
      ...
      renderItem={(item) => (
        <List.Item>
          <Typography.Text mark>{`[mock名称]`}</Typography.Text>{'\u00A0'}{'\u00A0'}{'\u00A0'}{'\u00A0'}{`[mock方法]`}
        </List.Item>
      )}
    />
      </Modal>
    ...
    <div className='mock-page'>
      <ModeContext.Provider value={{ ...props }}>
        ...
        {mode === 'mock' ? <Button type='primary' style={{margin:'10px 0'}} onClick={openModal}>mock列表</Button> : ''}
        <MockReqParams />
        <MockResponse />
